...
# player_game is for backtracking algorithm and player. The game will be start from 0's.
...
# Every side can have same point. So, we are checking if it used before.
        for k in range(len(side_arr)):
            for l in side_arr[k]:
                if (l in c_arr):
                    side_arr[k].remove(l)
        # Get a random queen's point.
        ...
        while(len(side_arr[choice])==0):
            m+=1
            choice = random.randint(0,game_size-1)
            # It is just trying to reduce bug rate. If it loops game_size+10 it will say there is not a choice and check it for every queen's point.
            if(m==game_size+10):
                for k in range(len(side_arr)):
                    if(len(side_arr[k]) != 0):
                        choice=k
        ...
        if(null):
            break
# colored = point which is colored. colors_loc stores all point according to queen's row.
...
# This will add side's side to choiced color sides.
...
def game():
    while(True):
        ...
        while (says_empty == 1 and win_or_not(x_coordinate, y_coordinate,table_arr,player_game) == False):
            says_empty = int(input(f"PLease choose valid number. if you mark cross write 0 if you mark a queen write 1: "))
        ...
        if(says_empty==1):
            player_game[x_coordinate][y_coordinate] = 1001
            game_over+=1
            if(game_over==9):
                break
        else:
...
# Second and working backtracking try.
...
# Randomly choosing a queen column for first row if it can't pass the mismovement(win_or_not) then it will go back.
        while  win_or_not(i, rand,table_arr,player_game) == False:
            ...
            if len(tried) == len(all_number):
                we_stop = True
                break
# We didn't stop so, we can loop the while and -1 for the impossible points.
        if(we_stop == False):
            # How many times we passed without any stop.
            passed+=1
            # count is counting the backwards from the table.
...
# We stopped for backward from the table. All history came back. and we will try new solution again.
        else:
            passed=0
            print(f"COUNT:> {count}")
...
###################################### YOU CAN RECOMMENT TO FIND ANY DUPLICATE COLOR IN THE BACKTRACKING ANSWER ###############################################
# answer = []
